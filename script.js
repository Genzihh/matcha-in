...
// Tambahkan variabel pendukung swipe di bagian atas
let touchStartX = 0;
let touchEndX = 0;
...
function handleSwipe() {
    const threshold = 50; // Jarak minimal geser (dalam pixel) untuk dianggap swipe
...
// Fungsi pembantu untuk navigasi manual
function nextSlideManual() {
    const slides = document.querySelectorAll(".hero-slide");
    let next = slideIndex >= slides.length ? 0 : slideIndex;
    currentSlide(next);
}

function prevSlideManual() {
    const slides = document.querySelectorAll(".hero-slide");
    let prev = slideIndex <= 1 ? slides.length - 1 : slideIndex - 2;
    currentSlide(prev);
}